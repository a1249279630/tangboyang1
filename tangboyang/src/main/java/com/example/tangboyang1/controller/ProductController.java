...
public class ProductController {
    ...
    public ResultDTO addProduct(@RequestBody AddProductsRequest addProductsRequest){

        Integer integer = productsService.addProducts(addProductsRequest);
        if(integer==1){
            return ResultDTO.ok("添加成功");
        }else if(integer==-1){
            throw new ErrorCodeException(CommonErrorCode.UNKOWN_ERROR);
        }else {
            return ResultDTO.fail("该用户不能上传商品，请申请权限");
        }
    }
